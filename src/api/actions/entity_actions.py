...
class EntityActions:
    ...
    def create_entity(self, entity: EntityRequest) -> EntityResponse:
        ...
        try:
            ...
            # В любом случае бросаем исключение — функция не может вернуть EntityResponse
    ...
    def create_entity_synthetic(self, entity: EntityRequest) -> EntityResponse:
        response = self.client.post(
            Endpoints.CREATE, json=entity.model_dump(exclude_none=True)
        )
        response.raise_for_status()
        ...
        try:
            ...
            if isinstance(data, int):
                ...
                # временный workaround — собрать EntityResponse вручную
                fixed = EntityResponse(id=data, **entity.model_dump())
                allure.attach(
    ...
    def get_all_entities(self, verified: Optional[bool] = None, page: Optional[int] = None, per_page: Optional[int] = None) -> list[EntityResponse]:
        params = {
    ...
    def patch_entity(self, entity_id: int, entity: EntityRequest) -> requests.Response:
